...
class HtpasswdVerifierTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testDes()
    {
        ...
        $this->assertTrue($this->verifier->verify('12345678', $encrypted));
        $this->assertFalse($this->verifier->verify('wrong', $encrypted));
        $this->assertFalse($this->verifier->verify('1234567890', $encrypted));
    }

    public function testSha()
    {
        $encrypted = '{SHA}MCdMR5A70brHYzu/CXQxSeurgF8=';
        $this->assertTrue($this->verifier->verify('passwd', $encrypted));
        $this->assertFalse($this->verifier->verify('wrong', $encrypted));
    }
    ...
    public function testApr()
    {
        ...
        $this->assertTrue($this->verifier->verify('tkirah', $encrypted));
        $this->assertFalse($this->verifier->verify('wrong', $encrypted));
    }
